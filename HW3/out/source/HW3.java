...
public class GameObject {
    ...
    public void Draw() {
        ...
        for (int i=0; i<mesh.triangles.size(); i++) {
            ...
            for (int y = PApplet.parseInt(minY); y < PApplet.parseInt(maxY); y++) {
                for (int x = PApplet.parseInt(minX); x < PApplet.parseInt(maxX); x++) {
                    ...
                    int index = y * PApplet.parseInt(renderer_size.z - renderer_size.x) + x;
...
class Mesh {
    ...
    public void addFace(int a, int at, int b, int bt, int c, int ct) {
        ...
        for (int i = 0; i < 3; i += 1) {
            Vector3 n = Vector3.cross(Vector3.sub(vs[(i + 1) % 3], vs[i]), Vector3.sub(vs[(i + 2) % 3], vs[i]))
                    .unit_vector();
            normal[i] = n;
        }
        if (uvs.size() >= 1) {
...
public class DepthVertexShader extends VertexShader {
    public Vector4[] main(Object[] attribute, Object[] uniform) {
        ...
        for (int i = 0; i < result.length; i++) {
            result[i] = MVP.mult(aVertexPosition[i].getVector4(1.0f));
        }
